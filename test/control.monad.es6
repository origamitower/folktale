...
describe('Curried', function() {
  ...
  property('ap', 'monad (json -> json)', 'monad json', env, function(mf, ma) {
  ...
  property('map', 'json -> json', 'monad json', env, function(f, ma) {
    return _.map(f)(ma).equals(ma.map(f))
  });

  property('of', 'json', 'monad json', env, function(a, ma) {
